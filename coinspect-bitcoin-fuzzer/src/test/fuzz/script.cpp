...
FUZZ_TARGET_INIT(script, initialize_script)
{
	...
	std::string input = std::string(reinterpret_cast<const char*>(buffer.data()), buffer.size());

	std::cout << input;

	std::size_t foundLockScript = input.find(lockScriptToken);
	std::size_t foundUnlockScript = input.find(unlockScriptToken);

	if (foundLockScript != std::string::npos)
		std::cout << "\nfirst 'Lock Script' found at: " << foundLockScript << '\n';
	if (foundUnlockScript != std::string::npos)
		std::cout << "first 'Unlock Script' found at: " << foundUnlockScript << '\n';

	foundLockScript = input.find(lockScriptToken, foundLockScript + lockScriptToken.length());
	foundUnlockScript = input.find(unlockScriptToken, foundUnlockScript + unlockScriptToken.length());

	if (foundLockScript != std::string::npos)
		std::cout << "second 'Lock Script' found at: " << foundLockScript << '\n';
	if (foundUnlockScript != std::string::npos)
		std::cout << "second 'Unlock Script' found at: " << foundUnlockScript << '\n';
	...
	IsStandard(scriptSig_deprecateERP, std::nullopt, which_type);
	check_type(which_type);
	...
	std::cout << "\n\n";
	IsStandard(scriptSig_newERP, std::nullopt, which_type);
	check_type(which_type);
#endif
